...
UBUNTU_TRANSLATION_DOMAIN="scriptor.newparadigmsoftware"
...
aptest.commands = QML2_IMPORT_PATH=$$OUT_PWD/backend bash $$PWD/Scriptor/tests/autopilot/run
aptest.depends  = sub-Scriptor sub-backend-App

unittest.target   = check
unittest.commands = /usr/bin/qmltestrunner -input $$PWD/backend/tests/unit -import $$OUT_PWD/backend
unittest.commands += && /usr/bin/qmltestrunner -input $$PWD/Scriptor/tests/unit -import $$OUT_PWD/backend
unittest.depends  = sub-Scriptor sub-backend-App
